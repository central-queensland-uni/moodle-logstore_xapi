...
/**
 * xAPI transformer for mod_spa group override deleted event.
 *
...
function group_override_deleted(array $config, \stdClass $event) {
    $repo = $config['repo'];
    $user = $repo->read_record_by_id('user', $event->userid);
    $lang = $config['source_lang'];

    $object = utils\get_activity\spa_group_override($config, $event->objectid);
    $object['id'] = $config['app_url'].'/mod/spa/overrides.php?cmid='.$event->objectid;
